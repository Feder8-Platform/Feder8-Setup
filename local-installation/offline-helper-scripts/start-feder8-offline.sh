...
if [ -f "images.tar" ] && [ -f "therapeutic-areas.json" ] && [ -f "feder8-local-images.json" ]; then
  ...
  docker run --rm -it --name feder8-installer -e CURRENT_DIRECTORY=$(pwd) -e IS_WINDOWS=false -e IS_MAC=$IS_MAC -e DOCKER_CERT_SUPPORT=$DOCKER_CERT_SUPPORT -v ./therapeutic-areas.json:$THERAPEUTIC_AREA_JSON_PATH -v ./feder8-local-images.json:$FEDER8_LOCAL_IMAGE_JSON_PATH -v /var/run/docker.sock:/var/run/docker.sock -v ./info_feder8_installation:/opt/install-script/info_feder8_installation ${REGISTRY}/library/install-script:${TAG} feder8 init --offline full
else

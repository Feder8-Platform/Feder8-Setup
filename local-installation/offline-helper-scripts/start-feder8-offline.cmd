...
if exist images.tar (
    if exist therapeutic-areas.json (
        if exist feder8-local-images.json (
            ...
            docker run --rm -it --name feder8-installer -e CURRENT_DIRECTORY=%CD% -e IS_WINDOWS=true -e DOCKER_CERT_SUPPORT=false -v ./therapeutic-areas.json:%THERAPEUTIC_AREA_JSON_PATH% -v ./feder8-local-images.json:%FEDER8_LOCAL_IMAGE_JSON_PATH% -v /var/run/docker.sock:/var/run/docker.sock -v %CD%/info_feder8_installation:/opt/install-script/info_feder8_installation %REGISTRY%/library/install-script:%TAG% feder8 init --offline full
        )
    )
) else (

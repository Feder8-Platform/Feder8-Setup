...
> **The evaluation runs against the `0.3.0` candidate image automatically.** The script pins
...
run needs it so the eval uses `0.3.0` and not your production image:

```bash
export CLINICAL_IMAGE=harbor.honeur.org/honeur/clinical-notes-processor:0.3.0
```
